...
namespace hmi
{
    touch::touch(touch_config config) : m_config(config)
    {
        ...
        if (!set_events(ui::ev_type::sync, ui::ev_type::key, ui::ev_type::abs))
            throw std::runtime_error{ "Failed to set supproted events." };

        if (!set_key_codes(ui::btn_code::touch))
            throw std::runtime_error{ "Failed to set supproted key codes." };

        if (!set_abs_codes(ui::abs_code::mt_x, ui::abs_code::mt_y, ui::abs_code::mt_track_id, ui::abs_code::mt_slot))
            throw std::runtime_error{ "Failed to set supproted abs codes." };

        set_abs_info<ui::abs_code::mt_x>(0, m_config.touch_width);
        set_abs_info<ui::abs_code::mt_y>(0, m_config.touch_height);
        set_abs_info<ui::abs_code::mt_track_id>(0, std::numeric_limits<std::int32_t>::max());
        set_abs_info<ui::abs_code::mt_slot>(0, m_config.max_slots);
    ...
    {
        return emit(ui::event_gen::touch_press_event(get_x(x), get_y(y), track_id, slot));
    }

    bool touch::release(const std::int32_t x, const std::int32_t y, const std::int32_t slot) noexcept
    {
        return emit(ui::event_gen::touch_release_event(get_x(x), get_y(y), slot));
    }

    bool touch::move(const std::int32_t x, const std::int32_t y, const std::int32_t track_id,
                     const std::int32_t slot) noexcept
    {
        return emit(ui::event_gen::touch_move_event(get_x(x), get_y(y), track_id, slot));
    }
