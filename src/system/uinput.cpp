...
{
    uinput::uinput()
        : fd::file_desc(uinput_path, fd::flag::write_only, fd::flag::non_block)
    {}

void uinput::set_info(std::string_view name, 
...
{
    return fd::file_desc::ioctl(bit::dev_setup, std::addressof(m_uinput_setup)) && 
           fd::file_desc::ioctl(bit::dev_create);
}
}